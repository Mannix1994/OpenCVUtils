 ...
 * 还是多通道，通道分量的类型uchar、int、float还是doule，mat中的每个元素所
...
void CompareMats::compare(const Mat mat1, const Mat mat2) {
    ...
    for (int i = 0; i < mat1.rows; i++) {
        //获取每行行首的指针
        auto ptrCols1 = const_cast<uchar *>(mat1.ptr<uchar>(i));
        auto ptrCols2 = const_cast<uchar *>(mat2.ptr<uchar>(i));
        for (int j = 0; j < mat1.cols; j++) {
            //获取(i,j)元素对应数组的指针
            uchar *a = ptrCols1+j*elemSize;
            uchar *b = ptrCols2+j*elemSize;
            //判断是否相等
            bool tag = true;
